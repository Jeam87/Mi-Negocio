...
<title>Mi Negocio 9.1</title><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
...
<div class="grid grid-cols-2 gap-2 mt-4"><button onclick="addInsumo()" class="text-[11px] bg-orange-100 py-3 rounded-full font-black">+ Ingrediente inventario</button><button onclick="addSalsa()" class="text-[11px] bg-amber-200 py-3 rounded-full font-black">+ Del recetario</button></div>
...
<div class="mt-6 bg-white rounded-[20px] p-4 shadow-xl border"><h3 class="font-bold text-sm">Ticket <span id="clienteSel" class="text-[10px] text-gray-400"></span></h3><select id="selCliente" class="w-full border-2 p-2 rounded-xl text-[12px] mt-2"><option value="">Cliente mostrador</option></select><div id="ticket" class="text-[12px] mt-2">Vacio</div><div class="flex justify-between font-black text-lg mt-3">Total $<span id="c-total">0</span></div><button onclick="cobrar()" class="w-full mt-3 bg-black text-white py-3 rounded-xl font-black">COBRAR -> Flujo</button><button onclick="enviarWA()" class="w-full mt-2 bg-green-500 text-white py-2 rounded-xl text-sm font-bold">WhatsApp presupuesto</button></div></div>

<div id="tab-inventario" class="p-3 hidden">
<div class="bg-[#2D3748] rounded-[24px] p-4 text-white"><h2 class="font-black text-[15px]">📦 Inventario maestro</h2><p class="text-[11px] opacity-70 mt-1">Si cambias precio aquí, todas las recetas se actualizan solas</p></div>
<div class="mt-3 bg-white rounded-[20px] p-4 shadow-sm">
<div class="grid grid-cols-5 gap-2"><input id="inv-nombre" placeholder="Ej: Papas" class="col-span-2 border-2 border-black p-3 rounded-xl font-bold text-[13px]"><input id="inv-precio" type="number" placeholder="$30" class="border-2 border-black p-3 rounded-xl font-black text-[13px]"><select id="inv-unidad" class="border-2 border-black p-3 rounded-xl text-[11px] font-bold"><option>kg</option><option>g</option><option>L</option><option>ml</option><option>pza</option><option>m</option></select><button onclick="addInventario()" class="bg-black text-white rounded-xl font-black text-xl">+</button></div>
...
<input id="buscInv" placeholder="🔍 Buscar..." class="w-full border-2 p-3 rounded-xl mt-3 text-[12px]" oninput="renderInventarioMaster()">
...
<div class="mt-3 flex gap-1 overflow-auto"><button onclick="setFin('flujo')" id="f-flujo" class="px-3 py-2 rounded-full text-[10px] font-black bg-white text-black">📊 Flujo</button><button onclick="setFin('facturas')" id="f-facturas" class="px-3 py-2 rounded-full text-[10px] bg-white/20">Facturas</button><button onclick="setFin('proveedores')" id="f-proveedores" class="px-3 py-2 rounded-full text-[10px] bg-white/20">Provee.</button><button onclick="setFin('categorias')" id="f-categorias" class="px-3 py-2 rounded-full text-[10px] bg-white/20">Cat.</button><button onclick="setFin('fijos')" id="f-fijos" class="px-3 py-2 rounded-full text-[10px] bg-white/20">Fijos</button></div></div>
...
<div class="bg-[#2D3748] rounded-[24px] p-4 text-white"><h2 class="font-black text-[15px]">👥 Clientes</h2><p class="text-[11px] opacity-70">Guarda tus clientes para presupuesto y ticket</p></div>
<div class="mt-3 bg-white rounded-[20px] p-4 shadow-sm">
<div class="flex gap-2"><input id="cliNombre" placeholder="Nombre cliente" class="flex-1 border-2 border-black p-3 rounded-xl font-bold text-[13px]"><input id="cliTel" placeholder="Tel" class="w-24 border-2 p-3 rounded-xl text-[12px]"><button onclick="addCliente()" class="bg-black text-white px-5 rounded-xl font-black text-xl">+</button></div>
<button onclick="addCliente()" class="w-full mt-2 bg-black text-white py-3 rounded-2xl font-black text-[13px]">GUARDAR CLIENTE</button>
<div id="cliMsg" class="hidden mt-3 p-2 rounded-xl text-center font-bold text-[12px]"></div>
<input id="buscCli" placeholder="🔍 Buscar cliente..." class="w-full border-2 p-3 rounded-xl mt-3 text-[12px]" oninput="renderClientes()">
<div id="listaClientes" class="mt-4 space-y-2"></div>
</div>
</div>
...
function addInventario(){
  ...
  if(ex){ ex.precio=p; ex.unidad=u; msg.className='mt-3 p-2 rounded-xl text-center font-bold bg-amber-100'; msg.innerText='✅ Actualizado: '+n; } else { inv.push({id:Date.now().toString(),nombre:n,precio:p,unidad:u}); msg.className='mt-3 p-2 rounded-xl text-center font-bold bg-green-100 text-green-700'; msg.innerText='✅ Guardado: '+n; }
...
function renderInventarioMaster(){
  ...
  let h=''; filtered.forEach((it)=>{
    ...
    h+=`<div class="flex gap-2 items-center bg-gray-50 p-3 rounded-xl border"><div class="flex-1"><b class="text-[13px]">${it.nombre}</b><br><span class="text-[11px] text-gray-500">$${it.precio} / ${it.unidad}</span></div><input type="number" value="${it.precio}" onchange="updateInvPrecio(${realIdx},this.value)" class="w-20 border-2 border-black p-2 rounded-xl font-black text-[13px]"><button onclick="delInv(${realIdx})" class="text-red-400 font-black px-2">X</button></div>`;
  });
  document.getElementById('listaInvMaster').innerHTML=h;
}
function updateInvPrecio(idx,val){ let inv=getInv(); inv[idx].precio=parseFloat(val)||0; localStorage.setItem('inventarioMaestro',JSON.stringify(inv)); calc(); }
...
// CLIENTES
function addCliente(){
  let n=document.getElementById('cliNombre').value.trim(), t=document.getElementById('cliTel').value.trim();
  let msg=document.getElementById('cliMsg');
  if(!n){ msg.className='mt-3 p-2 rounded-xl text-center font-bold bg-red-100 text-red-700'; msg.innerText='Pon nombre'; msg.classList.remove('hidden'); return; }
  let cli=getCli(); cli.push({id:Date.now().toString(),nombre:n,tel:t});
  ...
  msg.className='mt-3 p-2 rounded-xl text-center font-bold bg-green-100 text-green-700'; msg.innerText='✅ Cliente '+n+' guardado'; msg.classList.remove('hidden'); setTimeout(()=>msg.classList.add('hidden'),2000);
  renderClientes(); renderClientesSel();
}
function renderClientes(){
  let cli=getCli(); let q=(document.getElementById('buscCli')?.value||'').toLowerCase();
  let filtered=cli.filter(x=>x.nombre.toLowerCase().includes(q));
  let h=''; filtered.forEach((c,i)=>{
    let realIdx=getCli().findIndex(x=>x.id==c.id);
    h+=`<div class="flex justify-between items-center bg-gray-50 p-3 rounded-xl border"><div><b class="text-[13px]">${c.nombre}</b><br><span class="text-[11px] text-gray-500">${c.tel||'Sin tel'}</span></div><div class="flex gap-2"><button onclick="if(confirm('¿Borrar ${c.nombre}?')){let cl=getCli(); cl.splice(${realIdx},1); localStorage.setItem('clientes',JSON.stringify(cl)); renderClientes(); renderClientesSel();}" class="text-red-400 font-black px-2">X</button></div></div>`;
  });
  document.getElementById('listaClientes').innerHTML=h||'<p class="text-center text-[11px] text-gray-400 py-4">Sin clientes</p>';
}
function renderClientesSel(){
  let cli=getCli(); let sel=document.getElementById('selCliente');
  if(sel) sel.innerHTML='<option value="">Cliente mostrador</option>'+cli.map(c=>`<option value="${c.id}">${c.nombre}</option>`).join('');
}
...
function addInsumo(d={}){
  ...
  <div class="bg-white rounded-xl p-2"><div class="grid grid-cols-3 gap-2"><input type="number" value="${d.cc||1}" class="in-cc w-full border-2 p-2 rounded-lg font-bold" oninput="calc()"><input value="${d.uc||'kg'}" class="in-uc w-full border-2 p-2 rounded-lg text-[11px] font-bold" readonly><input type="number" value="${d.pc||''}" class="in-pc w-full border-2 bg-amber-50 p-2 rounded-lg font-bold" oninput="calc()"></div><div class="grid grid-cols-3 gap-2 mt-2"><input type="number" value="${d.cu||''}" placeholder="Uso" class="in-cu w-full border-2 border-black p-2 rounded-lg font-bold" oninput="calc()"><select class="in-uu w-full border-2 border-black p-2 rounded-lg text-[11px]" onchange="calc()"><option>kg</option><option>g</option><option>L</option><option>ml</option><option>pza</option></select><div class="bg-black text-white rounded-lg p-2 text-center"><div class="text-[8px] opacity-70">ME SALE</div><div class="font-black text-[12px]">$<span class="in-sub">0.00</span></div></div></div></div>`;
...
function cambiarInv(sel){
  ...
  row.querySelector('.in-pc').value=parseFloat(opt.dataset.p)||0;
  row.querySelector('.in-uc').value=opt.dataset.u||'kg';
  let uu=row.querySelector('.in-uu'); let u=opt.dataset.u; if(u=='kg') uu.value='g'; if(u=='L') uu.value='ml';
  calc();
}
function addSalsa(){ let bases=getCats().filter(c=>c.grupo=='recetario').map(c=>c.id); let ss=getProd().filter(p=>bases.includes(p.tipo)); if(!ss.length) return alert('Crea recetario primero'); let div=document.createElement('div'); div.className='bg-amber-50 p-3 rounded-[16px] border-2 border-amber-300'; div.dataset.t='rec'; div.innerHTML=`<div class="flex justify-between"><span class="font-black text-[11px]">📖 Del recetario</span><button onclick="this.parentElement.parentElement.remove();calc()" class="text-red-400">x</button></div><div class="grid grid-cols-4 gap-2 mt-2 bg-white p-2 rounded-xl"><select class="in-sel col-span-3 border-2 p-2 rounded-lg text-[12px] font-bold" onchange="calc()">${ss.map(s=>`<option value="${s.id}" data-c="${s.costo}">${s.nombre} $${s.costo.toFixed(2)}</option>`).join('')}</select><input type="number" value="1" class="in-cant border-2 p-2 rounded-lg font-bold" oninput="calc()"><div class="col-span-4 text-right font-black text-[12px]">Me sale: $<span class="in-sub">0.00</span></div></div>`; document.getElementById('insumos').appendChild(div); calc(); }
function addComp(){ let ps=getProd(); if(!ps.length) return alert('No hay'); let div=document.createElement('div'); div.className='bg-blue-50 p-3 rounded-[16px] border-2 border-blue-200'; div.dataset.t='cat'; div.innerHTML=`<div class="flex justify-between"><span class="font-black text-[11px]">🛒 Complemento</span><button onclick="this.parentElement.parentElement.remove();calc()" class="text-red-400">x</button></div><div class="grid grid-cols-4 gap-2 mt-2 bg-white p-2 rounded-xl"><select class="in-sel col-span-3 border-2 p-2 rounded-lg text-[12px] font-bold" onchange="calc()">${ps.map(s=>`<option value="${s.id}" data-c="${s.venta}">${s.nombre} $${s.venta.toFixed(2)}</option>`).join('')}</select><input type="number" value="1" class="in-cant border-2 p-2 rounded-lg font-bold" oninput="calc()"><div class="col-span-4 text-right font-black text-[12px]">$<span class="in-sub">0.00</span></div></div>`; document.getElementById('insumos').appendChild(div); calc(); }
...
function calc(){
  ...
  document.querySelectorAll('#insumos > div').forEach(row=>{
    ...
    if(row.dataset.t=='rec'||row.dataset.t=='cat'){
      ...
      let c=parseFloat(sel.options[sel.selectedIndex].dataset.c)||0; let cant=parseFloat(row.querySelector('.in-cant').value)||0; let s=c*cant; sub.innerText=s.toFixed(2); tot+=s;
    } else {
      ...
      else if((uc=='kg'&&uu=='g')||(uc=='L'&&uu=='ml')) cost=(pc/(cc*1000))*cu;
      else cost=(pc/cc)*cu;
...
function guardarProd(){
  ...
  let ings=[]; document.querySelectorAll('#insumos > div').forEach(row=>{
    ...
    else { let sel=row.querySelector('.in-n'); ings.push({tipo:'ing',invId:row.dataset.invId||sel.value,cc:row.querySelector('.in-cc').value,uc:row.querySelector('.in-uc').value,pc:row.querySelector('.in-pc').value,cu:row.querySelector('.in-cu').value,uu:row.querySelector('.in-uu').value}); }
  });
  let ps=getProd();
...
function renderInventario(){ let ps=getProd(); let cs=getCats(); let h=''; ps.forEach((p,i)=>{ let cat=cs.find(c=>c.id==p.tipo); h+=`<div class="bg-white p-3 rounded-2xl flex gap-2 shadow-sm mt-3 border items-center"><span class="text-[8px] bg-gray-100 px-2 py-1 rounded-full font-bold">${cat?cat.nombre:p.tipo}</span><div class="flex-1"><b class="text-[13px]">${p.nombre}</b><br><span class="text-[11px]">$${p.costo.toFixed(2)} → $${p.venta.toFixed(2)}</span></div><button onclick="editarProd(${p.id})" class="bg-blue-50 text-blue-600 px-3 py-1.5 rounded-full text-[11px] font-black">Editar</button><button onclick="if(confirm('¿Borrar?')){let pr=getProd(); pr.splice(${i},1); localStorage.setItem('productosV2',JSON.stringify(pr)); renderInventario(); renderVenta();}" class="text-red-400 ml-1 font-black">X</button></div>`; }); document.getElementById('listaInv').innerHTML=h; }
function editarProd(id){ let p=getProd().find(x=>x.id==id); if(!p) return; editId=id; tipoAct=p.tipo; document.getElementById('nombre').value=p.nombre; document.getElementById('insumos').innerHTML=''; if(p.ingredientes){ p.ingredientes.forEach(ing=>{ if(ing.tipo=='ing') addInsumo({invId:ing.invId,cc:ing.cc,uc:ing.uc,pc:ing.pc,cu:ing.cu,uu:ing.uu}); else if(ing.tipo=='rec'){ addSalsa(); let l=document.getElementById('insumos').lastElementChild; l.querySelector('.in-sel').value=ing.id; l.querySelector('.in-cant').value=ing.cant; } else if(ing.tipo=='cat'){ addComp(); let l=document.getElementById('insumos').lastElementChild; l.querySelector('.in-sel').value=ing.id; l.querySelector('.in-cant').value=ing.cant; } }); } showTab('costos'); renderBotones(); calc(); document.getElementById('btnCancel').classList.remove('hidden'); }
...
function cobrar(){ if(!carrito.length) return alert('Vacio'); let tot=parseFloat(document.getElementById('c-total').innerText); let facts=getFacts(); let now=new Date(); let iso=now.toISOString().split('T')[0]; let cliId=document.getElementById('selCliente').value; let cli=getCli().find(c=>c.id==cliId); facts.push({id:Date.now(),concepto:'Venta'+(cli?' a '+cli.nombre:'')+': '+carrito.map(c=>c.nombre+' x'+c.qty).join(', '),monto:tot,fecha:iso,fechaObj:now.getTime(),tipo:'entrada',origen:'venta'}); localStorage.setItem('facturas',JSON.stringify(facts)); carrito=[]; document.getElementById('c-total').innerText='0'; document.getElementById('ticket').innerHTML='Vacio'; finView='flujo'; showTab('finanzas'); }
function enviarWA(){ if(!carrito.length) return alert('Vacio'); let cliId=document.getElementById('selCliente').value; let cli=getCli().find(c=>c.id==cliId); let texto=`*PRESUPUESTO* ${cli?cli.nombre:''}%0A`; carrito.forEach(p=>texto+=`• ${p.nombre} x${p.qty} $${(p.venta*p.qty).toFixed(2)}%0A`); texto+=`*TOTAL $${document.getElementById('c-total').innerText}*`; window.open('https://wa.me/?text='+texto,'_blank'); }
...
renderBotones(); calcFijos(); addInsumo({}); calc(); renderInventario(); renderVenta(); renderFinanzas(); renderInventarioMaster(); renderClientes(); renderClientesSel(); showTab('clientes');
</script></body></html>
"""
