...
function normalizarUnidad(u){
 ...
 const m={g:['g','gr'],kg:['kg','kilo'],lb:['lb'],oz:['oz'],ml:['ml'],L:['l','lt','L'],gal:['gal'],m:['m'],cm:['cm'],pza:['pz','pza','pieza']};
 for(let k in m){ if(m[k].includes(u)||k===u) return k; }
 return u||'g';
}
function factorABase(u){ u=normalizarUnidad(u); let f={g:1,kg:1000,lb:453.592,oz:28.3495,ml:1,L:1000,gal:3785.41,m:100,cm:1,pza:1}; return f[u]||1; }
function convertir(c,de,a){
 de=normalizarUnidad(de); a=normalizarUnidad(a);
 if(de===a) return c;
 let peso=['g','kg','lb','oz'], vol=['ml','L','gal'];
 if(peso.includes(de)&&peso.includes(a)) return c*factorABase(de)/factorABase(a);
 if(vol.includes(de)&&vol.includes(a)) return c*factorABase(de)/factorABase(a);
 if(a==='pza'||de==='pza') return c; return c;
}
...
function addIng(val={}){
 ...
 div.innerHTML=`<input class="i-nombre flex-1 border-2 border-black rounded-xl p-2" placeholder="Ingrediente" value="${val.nombre||''}"><input class="i-cant w-[70px] border-2 border-black rounded-xl p-2" value="${val.cant||'1'}" oninput="calcBase()"><select class="i-unit w-[70px] border-2 border-black rounded-xl p-2"><option ${val.unit=='g'?'selected':''}>g</option><option ${val.unit=='kg'?'selected':''}>kg</option><option>ml</option><option>L</option><option>pza</option></select><input class="i-precio w-[80px] border-2 border-black rounded-xl p-2" placeholder="$" data-precio="${val.precio||0}" value="${val.precio||''}" oninput="this.dataset.precio=this.value;calcBase()"><button onclick="this.parentElement.remove();calcBase()" class="text-red-600 font-black">X</button>`;
...
function guardarBase(){
 ...
 if(editId){
   let b=prods.find(x=>x.id==editId);
   b.nombre=nombre; b.costo=costo; b.rendimiento={cant:rendCant,unit:rendUnit};
   editId=null;
 } else {
...
function editarBase(id){
 ...
 editId=id;
 render();
 document.getElementById('nombreBase').value=b.nombre;
 document.getElementById('rendCant').value=b.rendimiento.cant;
 document.getElementById('rendUnit').value=b.rendimiento.unit;
 document.getElementById('costoBase').innerText=b.costo.toFixed(2);
 document.getElementById('btnBase').innerText='ACTUALIZAR BASE';
 window.scrollTo(0,0);
}
function borrarProd(id){
 if(!confirm('¿Borrar?')) return;
 prods=prods.filter(x=>x.id!=id); save(); render();
}

// PRODUCTOS CON GRAMOS EDITABLE
function addBase(val={}){
 let bases=getProd().filter(x=>x.tipo==='base');
 if(bases.length===0){ alert('Primero crea una base'); return; }
 let opts=bases.map(b=>`<option value="${b.id}" ${val.id==b.id?'selected':''}>${b.nombre} - $${b.costo.toFixed(2)} - rinde ${b.rendimiento.cant} ${b.rendimiento.unit}</option>`).join('');
 let div=document.createElement('div');
 div.className='flex gap-2 items-center flex-wrap bg-yellow-50 p-2 rounded-xl border-2 border-black mb-2';
 div.innerHTML=`
  <select class="b-sel flex-1 min-w-[140px] border-2 border-black rounded-xl p-2" onchange="calc2()"><option value="">-- Receta --</option>${opts}</select>
  <input class="b-cant w-[80px] border-2 border-black rounded-xl p-2" value="${val.cant||'200'}" oninput="calc2()">
  <select class="b-unit w-[80px] border-2 border-black rounded-xl p-2" onchange="calc2()">
   <option value="g" ${val.unit=='g'?'selected':''}>g</option><option value="kg" ${val.unit=='kg'?'selected':''}>kg</option><option value="lb">lb</option><option value="ml">ml</option><option value="L">L</option><option value="pza" ${val.unit=='pza'?'selected':''}>pza</option>
  </select>
...
function guardarProd(){
 ...
 if(editId){
   let p=prods.find(x=>x.id==editId);
...
function editarProd(id){
 let p=prods.find(x=>x.id==id); if(!p) return;
 editId=id;
 render();
 document.getElementById('nombre2').value=p.nombre;
 document.getElementById('categoria2').value=p.categoria||'General';
 document.getElementById('basesSel').innerHTML='';
 p.bases.forEach(b=>addBase(b));
 calc2();
 document.getElementById('btnProd').innerText='ACTUALIZAR PRODUCTO';
...
function render(){
 ...
 <div class="bg-white border-[3px] border-black rounded-[20px] p-3 shadow-[4px_4px_0_#000]">
  ...
  <button id="btnBase" onclick="guardarBase()" class="w-full mt-2 bg-black text-white rounded-xl p-3 font-black">GUARDAR BASE</button>
  <div class="mt-3 space-y-1">
   ${bases.map(b=>`<div class="flex justify-between items-center bg-gray-100 p-2 rounded-xl border-2 border-black"><div class="text-xs"><b>${b.nombre}</b><br>$${b.costo.toFixed(2)} rinde ${b.rendimiento.cant} ${b.rendimiento.unit}</div><div class="flex gap-1"><button onclick="editarBase('${b.id}')" class="bg-yellow-300 border-2 border-black rounded-lg px-2 py-1 text-xs font-black">EDITAR</button><button onclick="borrarProd('${b.id}')" class="bg-red-500 text-white border-2 border-black rounded-lg px-2 py-1 text-xs font-black">X</button></div></div>`).join('')||'<div class="text-xs">Sin bases</div>'}
  </div>
 </div>

 <div class="bg-white border-[3px] border-black rounded-[20px] p-3 shadow-[4px_4px_0_#000]">
  <h2 class="font-black">2) Producto + Categoría (elige gramos)</h2>
  ...
  <button onclick="addBase()" class="w-full mt-2 bg-yellow-300 border-2 border-black rounded-xl p-3 font-black">+ AGREGAR RECETA (elige gramos)</button>
  ...
  <button id="btnProd" onclick="guardarProd()" class="w-full mt-3 p-4 bg-black text-white rounded-2xl font-black">GUARDAR PRODUCTO</button>
  <div class="mt-3 space-y-1">
   ${sec.map(p=>`<div class="flex justify-between items-center bg-yellow-50 p-2 rounded-xl border-2 border-black"><div class="text-xs"><b>${p.nombre}</b> [${p.categoria}]<br>$${p.costo.toFixed(2)} -> $${p.venta.toFixed(2)}<br><span class="text-[10px]">${p.bases.map(b=>{let rb=getProd().find(x=>x.id==b.id); return (rb?rb.nombre:b.id)+' '+b.cant+' '+b.unit}).join(', ')}</span></div><div class="flex gap-1"><button onclick="editarProd('${p.id}')" class="bg-yellow-300 border-2 border-black rounded-lg px-2 py-1 text-xs font-black">EDITAR</button><button onclick="borrarProd('${p.id}')" class="bg-red-500 text-white border-2 border-black rounded-lg px-2 py-1 text-xs font-black">X</button></div></div>`).join('')||'<div class="text-xs">Sin productos</div>'}
  </div>
 </div>`;
 calcBase(); calc2();
}
