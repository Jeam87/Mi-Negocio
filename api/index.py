...
<title>Mi Negocio 9.4 Fotos + Carrito editable</title><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
...
<div class="bg-white p-3 flex justify-between items-center sticky top-0 z-20 shadow-sm"><h1 class="font-black">Mi Negocio 9.4</h1><button onclick="showTab('config')" class="text-[10px] bg-gray-100 px-3 py-1 rounded-full">Config</button></div>
...
<button onclick="setCrear('base')" class="w-full mt-5 bg-[#FFF8F0] border-2 border-black rounded-[20px] p-5 text-left flex gap-4 items-center"><div class="w-14 h-14 bg-orange-200 rounded-2xl flex items-center justify-center text-xl">🧑‍🍳</div><div><b class="text-[15px]">1. Crear BASE</b><br><span class="text-[11px]">Salsas con rendimiento</span></div></button>
<button onclick="setCrear('producto')" class="w-full mt-3 bg-[#0F172A] rounded-[20px] p-5 text-left flex gap-4 items-center text-white"><div class="w-14 h-14 bg-[#4FD1C5] rounded-2xl flex items-center justify-center text-xl">🛍️</div><div><b class="text-[15px]">2. Crear PRODUCTO PARA VENDER con FOTO</b></div></button>
</div>
...
<h2 class="font-black text-[16px]">🧑‍🍳 Crear BASE</h2>
...
<button onclick="addInsumo()" class="w-full mt-3 bg-orange-100 border-2 py-3 rounded-2xl font-black text-[12px]">+ Ingrediente</button>
<div class="mt-4 bg-amber-50 border-2 border-amber-300 rounded-2xl p-3"><p class="font-black text-[12px]">¿Cuánto rinde?</p><div class="flex gap-2 mt-2"><input id="rendCant" type="number" value="10" class="flex-1 border-2 border-black p-3 rounded-xl font-black" oninput="calc()"><select id="rendUni" class="border-2 border-black p-3 rounded-xl font-bold text-[12px]" onchange="calc()"><option>L</option><option>ml</option><option>kg</option><option>g</option><option>pza</option></select></div></div>
<div class="mt-3 p-4 bg-[#0F172A] text-white rounded-[16px]"><div class="flex justify-between text-[13px]"><span>Ingredientes</span><b>$<span id="c-ing">0.00</span></b></div><div class="flex justify-between text-amber-300 text-[12px]"><span>+ Fijos</span><b>$<span id="c-fijos">0.00</span></b></div><div class="font-black text-[16px] border-t border-white/20 mt-2 pt-2"><div class="flex justify-between"><span>Total por <span id="r-cant-label">10</span><span id="r-uni-label">L</span>:</span><span>$<span id="costo">0.00</span></span></div><div class="text-[11px] text-green-300">Costo por 1 <span id="r-uni-label2">L</span>: $<span id="costo-unit">0.00</span></div><div class="flex justify-between mt-2 text-[#4FD1C5]"><span>Venta:</span><span>$<span id="venta">0.00</span></span></div></div><div class="mt-3 flex gap-2 bg-white/10 p-2 rounded-xl"><span class="text-[10px]">Margen %</span><input id="margen" type="number" value="50" class="w-16 text-black rounded-lg text-center font-black py-2" oninput="calc()"><input id="ventaManual" type="number" placeholder="$ final" class="flex-1 text-black rounded-lg px-2 py-2 font-black" oninput="calc()"></div></div>
...
<button onclick="setCrear('menu')" class="mb-3 text-[12px] font-bold">← Volver</button>
<div class="bg-white rounded-[28px] p-4 shadow-sm">
<h2 class="font-black text-[16px]">🛍️ Producto para vender</h2>
...
<div class="mt-3"><p class="font-black text-[11px]">📸 Foto del producto (opcional pero recomendado)</p><input type="file" id="fotoInput" accept="image/*" onchange="previewFoto(this)" class="w-full mt-2 text-[12px]"><div id="fotoPreview" class="mt-2 hidden"><img id="fotoImg" class="w-24 h-24 object-cover rounded-xl border-2 border-black"><button onclick="quitarFoto()" class="text-[10px] text-red-500 font-bold ml-2">Quitar foto X</button></div></div>
...
<button onclick="guardarProd('catalogo')" class="w-full mt-4 bg-black text-white py-4 rounded-2xl font-black">GUARDAR PRODUCTO CON FOTO</button>
</div>
</div>
</div>
...
<select id="selCliente" class="w-full border-2 border-black p-3 rounded-xl text-[12px] font-bold"><option value="">Mostrador</option></select>
<div id="ticket" class="mt-4 space-y-2">Vacio</div>
<div class="flex justify-between font-black text-[20px] mt-4 border-t-2 pt-3">Total $ <span id="c-total">0</span></div>
<button onclick="cobrar()" class="w-full mt-3 bg-black text-white py-4 rounded-2xl font-black text-[16px]">COBRAR</button>
<button onclick="limpiarCarrito()" class="w-full mt-2 bg-gray-100 py-2 rounded-xl text-[11px] font-bold">Vaciar carrito</button>
</div>
</div>
...
<script>
let editId=null, carrito=[], finView='flujo', fotoTemp='';
...
function getInv(){ let inv=JSON.parse(localStorage.getItem('inventarioMaestro')||'[]'); if(!inv.length){ inv=[{id:'1',nombre:'Papas',precio:30,unidad:'kg'}]; localStorage.setItem('inventarioMaestro',JSON.stringify(inv)); } return inv; }
...
function setCrear(v){
  ...
  if(v=='producto'){ document.getElementById('basesSel').innerHTML=''; document.getElementById('extrasSel').innerHTML=''; fotoTemp=''; document.getElementById('fotoPreview').classList.add('hidden'); document.getElementById('fotoInput').value=''; addBase(); calc2(); }
}
...
function addInsumo(d={}){
  ...
  div.innerHTML=`<select class="in-n w-full bg-white border-2 border-black p-2 rounded-xl font-bold text-[13px]" onchange="cambiarInv(this)"><option value="">-- Ingrediente --</option>${opts}</select><div class="grid grid-cols-3 gap-2 mt-2 bg-white p-2 rounded-xl"><input type="number" value="${d.cu||''}" placeholder="Uso" class="in-cu col-span-2 border-2 border-black p-2 rounded-lg font-bold" oninput="calc()"><select class="in-uu border-2 border-black p-2 rounded-lg text-[11px]" onchange="calc()"><option>g</option><option>kg</option><option>ml</option><option>L</option><option>pza</option><option>m</option></select><div class="col-span-3 text-right font-black text-[12px]">Me sale: $<span class="in-sub">0.00</span></div></div><input type="hidden" class="in-pc"><input type="hidden" class="in-uc"><button onclick="this.parentElement.remove();calc()" class="w-full mt-2 text-[10px] text-red-400">Quitar</button>`;
...
function calc(){
  try{
    let tot=0; let rc=parseFloat(document.getElementById('rendCant').value)||1; let ru=document.getElementById('rendUni').value;
    ...
    document.querySelectorAll('#insumos > div').forEach(row=>{
      ...
      let baseCost=0; if(uc=='kg'||uc=='L'){ baseCost=(pc/1000)*toBase(cu,uu); } else if(uc=='g'||uc=='ml'){ baseCost=(pc/1000)*cu; if(uu=='kg'||uu=='L') baseCost=pc*cu; } else if(uc=='pza'){ baseCost=pc*cu; } else if(uc=='m'){ baseCost=(pc/100)*toBase(cu,uu); } else baseCost=pc*cu;
      if(uc=='kg'&&uu=='kg') baseCost=pc*cu; if(uc=='L'&&uu=='L') baseCost=pc*cu; if(uc=='pza') baseCost=pc*cu;
      row.querySelector('.in-sub').innerText=baseCost.toFixed(2); tot+=baseCost;
    });
    document.getElementById('c-ing').innerText=tot.toFixed(2);
    let gf=parseFloat(document.getElementById('c-fijos')?.innerText||'0')||0; let total=tot+gf;
    ...
    let ventaEl=document.getElementById('venta'); if(vm&&parseFloat(vm)>0){ ventaEl.innerText=parseFloat(vm).toFixed(2); } else { ventaEl.innerText=(total*(1+m/100)).toFixed(2); }
  }catch(e){}
}
function addBase(){
...
function calc2(){
  try{
    ...
    document.querySelectorAll('#basesSel > div').forEach(r=>{
      ...
      let costoBase=parseFloat(sel.options[sel.selectedIndex].dataset.costo)||0; let rendCant=parseFloat(sel.options[sel.selectedIndex].dataset.cant)||1; let rendUni=sel.options[sel.selectedIndex].dataset.uni||'pza';
      let cantUsada=parseFloat(r.querySelector('.b-cant-used').value)||0; let uniUsada=r.querySelector('.b-uni-used').value;
      let costoProp=rendCant>0? (costoBase/toBase(rendCant,rendUni))*toBase(cantUsada,uniUsada) : 0;
...
function guardarProd(tipo){
  ...
  if(costo==0){ alert('Agrega ingredientes'); return; }
  ...
  let ps=getProd();
  let fotoToSave = isBase? '' : (fotoTemp || (editId? (ps.find(x=>x.id==editId)?.foto||'') : ''));
  if(editId){ let idx=ps.findIndex(x=>x.id==editId); if(idx>=0){ ps[idx].nombre=nom; ps[idx].costo=costo; ps[idx].venta=venta; ps[idx].tipo=tipoId; ps[idx].ingredientes=ings; if(rendimiento) ps[idx].rendimiento=rendimiento; if(!isBase) ps[idx].foto=fotoToSave; } }
  else { ps.push({id:Date.now(),tipo:tipoId,nombre:nom,costo,venta,ingredientes:ings,rendimiento,foto:fotoToSave}); }
...
function cancelEdit(){ editId=null; fotoTemp=''; document.getElementById('nombre').value=''; document.getElementById('nombreProd').value=''; document.getElementById('insumos').innerHTML=''; document.getElementById('ventaManual').value=''; document.getElementById('ventaManual2').value=''; document.getElementById('fotoPreview').classList.add('hidden'); document.getElementById('btnCancel').classList.add('hidden'); setCrear('menu'); }
function renderVenta(){
  let cs=getCats(); let ids=cs.filter(c=>c.grupo=='catalogo').map(c=>c.id); let ps=getProd().filter(p=>ids.includes(p.tipo));
  let h=''; ps.forEach(p=>{
    let fotoHtml=p.foto?`<img src="${p.foto}" class="w-full h-20 object-cover rounded-t-2xl">`:`<div class="w-full h-20 bg-gray-100 rounded-t-2xl flex items-center justify-center text-[20px]">🍗</div>`;
    h+=`<div class="bg-white rounded-2xl shadow-sm border overflow-hidden flex flex-col">${fotoHtml}<button onclick="addCart(${p.id})" class="w-full text-left p-3 flex-1"><b class="text-[13px] leading-tight block">${p.nombre}</b><p class="text-green-600 font-black text-[14px]">$${p.venta.toFixed(2)}</p><p class="text-[9px] text-gray-400">Costo $${p.costo.toFixed(2)}</p></button></div>`;
  });
  document.getElementById('listaVenta').innerHTML=h||'<p class="text-center text-gray-400 py-10 col-span-2">Crea productos con foto</p>';
}
...
function cobrar(){ if(!carrito.length) return alert('Vacio'); let tot=parseFloat(document.getElementById('c-total').innerText); let facts=getFacts(); let now=new Date(); let iso=now.toISOString().split('T')[0]; facts.push({id:Date.now(),concepto:'Venta: '+carrito.map(c=>c.nombre+' x'+c.qty).join(', '),monto:tot,fecha:iso,fechaObj:now.getTime(),tipo:'entrada'}); localStorage.setItem('facturas',JSON.stringify(facts)); carrito=[]; renderCarrito(); finView='flujo'; showTab('finanzas'); }
