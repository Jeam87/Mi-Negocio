...
def manifest():
    return jsonify({"name": "Mi Negocio 9.9.5","short_name": "Mi Negocio","start_url": "/","display": "standalone","icons": [{"src": "/logo.png","sizes": "512x512","type": "image/png"}]})
...
<title>Mi Negocio 9.9.5 Edit</title><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
...
<div id="tab-inventario" class="p-3 hidden"><div class="bg-white rounded-[20px] p-4"><div class="grid grid-cols-5 gap-2"><input id="inv-nombre" placeholder="Papas" class="col-span-2 border-2 border-black p-3 rounded-xl font-bold"><input id="inv-precio" type="number" placeholder="$30" class="border-2 border-black p-3 rounded-xl font-black"><select id="inv-unidad" class="border-2 border-black p-3 rounded-xl text-[10px] font-bold"><option>kg</option><option>g</option><option>gr</option><option>mg</option><option>L</option><option>ml</option><option>litro</option><option>pza</option><option>m</option><option>cm</option><option>mm</option><option>lb</option><option>libra</option><option>oz</option></select><button onclick="addInventario()" class="bg-black text-white rounded-xl font-black">+</button></div><div id="listaInvMaster" class="mt-4"></div></div></div>
<div id="tab-finanzas" class="p-3 hidden"><div class="bg-[#2D3748] rounded-[24px] p-4 text-white"><div class="flex justify-between"><h2 class="font-black">Finanzas</h2><div class="flex gap-2"><button onclick="openFiltros()" class="bg-white text-black px-3 py-2 rounded-xl font-black text-[11px]">Filtros</button><button onclick="openGasto()" class="bg-[#4FD1C5] text-black w-10 h-10 rounded-xl font-black text-xl">+</button></div></div><div class="grid grid-cols-3 gap-2 mt-3"><div class="bg-white/10 rounded-xl p-2 text-center"><p class="text-[9px] opacity-60">BALANCE</p><p class="font-black text-[14px]" id="fin-balance">$0</p></div><div class="bg-green-500/20 rounded-xl p-2 text-center"><p class="text-[9px]">ENTRADAS</p><p class="font-black text-[14px] text-green-300" id="fin-entradas">$0</p></div><div class="bg-red-500/20 rounded-xl p-2 text-center"><p class="text-[9px]">SALIDAS</p><p class="font-black text-[14px] text-red-300" id="fin-salidas">$0</p></div></div><div id="filtroActivo" class="hidden mt-3 bg-[#4FD1C5]/20 rounded-xl p-2 text-[11px] font-bold"></div></div><div class="mt-3 bg-white rounded-[20px] p-3"><div id="flu-lista" class="space-y-2"></div></div></div>
...
function getEmp(){ return JSON.parse(localStorage.getItem('empresaConfig')||'{"nombre":"Mi Negocio","logo":"","mostrarLogo":true,"mostrarFondo":true,"opacidad":8}'); }
function getCategoriasVenta(){ let cats=JSON.parse(localStorage.getItem('categoriasVenta')||'[]'); if(!cats.length){ cats=[{id:'todas',nombre:'Todas'},{id:'alitas',nombre:'Alitas'},{id:'papas',nombre:'Papas'},{id:'bebidas',nombre:'Bebidas'}]; localStorage.setItem('categoriasVenta',JSON.stringify(cats)); } return cats; }
function getLotes(){ return parseInt(localStorage.getItem('lotesMes')||'30')||30; }
function normalizarUnidad(u){ u=(u||'').toLowerCase().trim(); if(['g','gr','gramo','gramos'].includes(u)) return 'g'; if(['kg','kilo','kilos'].includes(u)) return 'kg'; if(['mg'].includes(u)) return 'mg'; if(['l','lt','litro','litros'].includes(u)) return 'L'; if(['ml'].includes(u)) return 'ml'; if(['m','mt','metro','metros'].includes(u)) return 'm'; if(['cm','centimetro'].includes(u)) return 'cm'; if(['mm'].includes(u)) return 'mm'; if(['lb','libra','libras'].includes(u)) return 'lb'; if(['oz','onza'].includes(u)) return 'oz'; if(['pza','pieza'].includes(u)) return 'pza'; return u; }
...
function familiaUnidad(u){ u=normalizarUnidad(u); if(['mg','g','kg','lb','oz'].includes(u)) return 'peso'; if(['ml','L'].includes(u)) return 'volumen'; if(['m','cm','mm'].includes(u)) return 'longitud'; return 'pieza'; }
function convertir(cant, de, a){ de=normalizarUnidad(de); a=normalizarUnidad(a); if(de==a) return cant; if(familiaUnidad(de)!=familiaUnidad(a)) return cant; return cant * factorABase(de) / factorABase(a); }

function addCategoriaVenta(){ let n=document.getElementById('nuevaCatNombre').value.trim(); if(!n) return; let cats=getCategoriasVenta(); let id=n.toLowerCase().replace(/\\s+/g,'-')+'-'+Date.now(); cats.push({id,nombre:n}); localStorage.setItem('categoriasVenta',JSON.stringify(cats)); document.getElementById('nuevaCatNombre').value=''; renderCategoriasVenta(); renderProdCategoriaSelect(); }
function addCategoriaVentaDesdeProd(){ let n=document.getElementById('quickCat').value.trim(); if(!n) return; let cats=getCategoriasVenta(); let id=n.toLowerCase().replace(/\\s+/g,'-')+'-'+Date.now(); cats.push({id,nombre:n}); localStorage.setItem('categoriasVenta',JSON.stringify(cats)); document.getElementById('quickCat').value=''; renderCategoriasVenta(); renderProdCategoriaSelect(); document.getElementById('prodCategoria').value=id; }
function editarCategoria(id){ let cats=getCategoriasVenta(); let cat=cats.find(c=>c.id==id); if(!cat) return; let nuevo=prompt('Nuevo nombre para categoría:', cat.nombre); if(!nuevo ||!nuevo.trim()) return; cat.nombre=nuevo.trim(); localStorage.setItem('categoriasVenta',JSON.stringify(cats)); renderCategoriasVenta(); renderProdCategoriaSelect(); renderInventario(); renderVenta(); }
function borrarCategoria(id){ if(id=='todas') return; if(!confirm('¿Borrar categoría? Los productos quedarán en General')) return; let cats=getCategoriasVenta().filter(c=>c.id!=id); localStorage.setItem('categoriasVenta',JSON.stringify(cats)); let prods=getProd(); prods.forEach(p=>{ if(p.categoria==id) p.categoria='general'; }); localStorage.setItem('productosV2',JSON.stringify(prods)); if(categoriaFiltro==id) categoriaFiltro='todas'; renderCategoriasVenta(); renderProdCategoriaSelect(); renderInventario(); renderVenta(); }
function renderCategoriasVenta(){
  let cats=getCategoriasVenta();
  let html=`<button onclick="filtrarCategoria('todas')" class="px-4 py-2 rounded-full font-black text-[11px] whitespace-nowrap border-2 ${categoriaFiltro=='todas'?'bg-black text-white border-black':'bg-white border-black'}">Todas</button>`;
  html+=cats.filter(c=>c.id!='todas').map(c=>`<button onclick="filtrarCategoria('${c.id}')" class="px-4 py-2 rounded-full font-bold text-[11px] whitespace-nowrap border-2 ${categoriaFiltro==c.id?'bg-black text-white border-black':'bg-white border-black'}">${c.nombre}</button>`).join('');
  document.getElementById('filtrosCats').innerHTML=html;
  let editHtml=cats.filter(c=>c.id!='todas').map(c=>`<div class="flex justify-between items-center bg-white p-2 rounded-xl border"><span class="text-[12px] font-bold">${c.nombre}</span><div class="flex gap-1"><button onclick="editarCategoria('${c.id}')" class="bg-blue-100 text-blue-600 px-2 py-1 rounded-full text-[10px] font-bold">✏️</button><button onclick="borrarCategoria('${c.id}')" class="bg-red-100 text-red-600 px-2 py-1 rounded-full text-[10px] font-bold">X</button></div></div>`).join('');
  document.getElementById('listaCatsEdit').innerHTML=editHtml||'<p class="text-[11px] text-gray-400">Sin categorías personalizadas</p>';
}
...
// RECETAS CON UNIDADES + EDITABLE
function addInsumo(d={}){
  let inv=getInv();
  let opts=inv.map(it=>`<option value="${it.id}" ${d.invId==it.id?'selected':''}>${it.nombre} $${it.precio}/${it.unidad}</option>`).join('');
  let div=document.createElement('div');
  div.className='bg-[#FFF8F0] p-3 rounded-[16px] border-2 border-orange-100';
  div.innerHTML=`
    <select class="in-n w-full bg-white border-2 border-black p-2 rounded-xl font-bold text-[13px]" onchange="calc()"><option value="">-- Ingrediente --</option>${opts}</select>
    <div class="grid grid-cols-5 gap-2 mt-2">
      <input type="number" value="${d.cu||''}" placeholder="Uso" class="in-cu col-span-2 border-2 border-black p-3 rounded-xl font-bold text-[13px]" oninput="calc()">
      <select class="in-uu col-span-3 border-2 border-black p-3 rounded-xl font-bold text-[12px]" onchange="calc()">
        <option value="kg">kg</option><option value="g">g / gr</option><option value="mg">mg</option><option value="L">L / litro</option><option value="ml">ml</option><option value="pza">pza</option><option value="m">m / metro</option><option value="cm">cm</option><option value="mm">mm</option><option value="lb">lb / libra</option><option value="oz">oz</option>
      </select>
    </div>
    <div class="text-right font-black text-[12px] mt-1">Costo: $<span class="in-sub">0.00</span> <span class="in-det text-[9px] text-gray-500 font-normal"></span></div>
    <button onclick="this.parentElement.remove();calc()" class="w-full mt-2 text-[10px] text-red-400">Quitar</button>`;
  document.getElementById('insumos').appendChild(div);
  if(d.uu) div.querySelector('.in-uu').value=d.uu; else div.querySelector('.in-uu').value='g';
}
function calc(){
  try{
    let tot=0;
    document.querySelectorAll('#insumos > div').forEach(row=>{
      ...
      let it=getInv().find(x=>x.id==invId);
      let cu=parseFloat(row.querySelector('.in-cu').value)||0;
      let uu=row.querySelector('.in-uu')?.value||'g';
      let baseCost=0, detalle='';
      if(it && cu){ let cantConvertida=convertir(cu, uu, it.unidad); baseCost=cantConvertida * it.precio; if(normalizarUnidad(uu)!=normalizarUnidad(it.unidad)) detalle=`(${cu}${uu} → ${cantConvertida.toFixed(3)}${it.unidad})`; }
      row.querySelector('.in-sub').innerText=baseCost.toFixed(2);
      let detEl=row.querySelector('.in-det'); if(detEl) detEl.innerText=detalle;
      tot+=baseCost;
    });
    document.getElementById('c-ing').innerText=tot.toFixed(2);
    let fijos=window._costoFijoPorLote||0;
    document.getElementById('c-fijos').innerText=fijos.toFixed(2);
    let rc=parseFloat(document.getElementById('rendCant').value)||1;
    document.getElementById('r-cant-label').innerText=rc;
    let total=tot+fijos;
    document.getElementById('costo').innerText=total.toFixed(2);
    document.getElementById('costo-unit').innerText=(rc>0?total/rc:0).toFixed(2);
    let m=parseFloat(document.getElementById('margen').value)||0;
    let vm=document.getElementById('ventaManual').value;
    document.getElementById('venta').innerText= vm? parseFloat(vm).toFixed(2) : (total*(1+m/100)).toFixed(2);
  }catch(e){}
}
function addBase(d={}){
  let bases=getProd();
  let opts=bases.map(b=>`<option value="${b.id}" ${d.id==b.id?'selected':''}>${b.nombre} $${b.costo.toFixed(2)}</option>`).join('');
  let div=document.createElement('div');
  div.className='bg-white border-2 border-black rounded-xl p-3 flex gap-2 items-center';
  div.innerHTML=`<select class="b-sel flex-1 border-2 p-2 rounded-lg font-bold text-[12px]" onchange="calc2()">${opts}</select><button onclick="this.parentElement.remove();calc2()" class="text-red-400 font-black px-2">X</button>`;
...
function guardarProd(tipo){
  ...
  let catId=isBase?'base':(document.getElementById('prodCategoria')?.value||'general');
  let ps=getProd();
  // recolectar receta
  let receta=[];
  if(isBase){
    document.querySelectorAll('#insumos > div').forEach(row=>{
      ...
      if(invId && cu) receta.push({invId, cu:parseFloat(cu), uu});
    });
  } else {
    document.querySelectorAll('#basesSel > div').forEach(row=>{
      let id=row.querySelector('.b-sel')?.value;
      if(id) receta.push({baseId:id});
    });
  }
  let rendCant=parseFloat(document.getElementById('rendCant')?.value)||10;
  ...
  if(editId){
    let idx=ps.findIndex(p=>p.id==editId);
    if(idx>=0){
      ps[idx].nombre=nom;
      ps[idx].costo=costo;
      ps[idx].venta=venta;
      ...
      ps[idx].rendCant=rendCant;
      ps[idx].rendUni=rendUni;
      ps[idx].margen=margen;
      if(fotoTemp) ps[idx].foto=fotoTemp;
    }
  } else {
    ps.push({id:Date.now(),tipo:'base',nombre:nom,costo,venta,foto:fotoTemp,categoria:catId,receta,rendCant,rendUni,margen,esBase:isBase});
  }
  localStorage.setItem('productosV2',JSON.stringify(ps));
  alert(editId?'✅ Actualizado':'✅ Guardado');
  fotoTemp=''; editId=null;
  renderInventario(); setCrear('menu'); renderVenta();
}

function editarProd(id){
  let p=getProd().find(x=>x.id==id); if(!p) return;
  editId=id;
  if(p.esBase!==false && (p.categoria=='base' || p.esBase)){
    // es base/receta
    setCrear('base');
    ...
    document.getElementById('rendCant').value=p.rendCant||10;
    document.getElementById('rendUni').value=p.rendUni||'L';
    document.getElementById('margen').value=p.margen||50;
    document.getElementById('insumos').innerHTML='';
    if(p.receta && p.receta.length){
      p.receta.forEach(r=> addInsumo({invId:r.invId, cu:r.cu, uu:r.uu}));
    } else { addInsumo({}); }
    document.getElementById('tituloBase').innerText='BASE - Editando: '+p.nombre;
    document.getElementById('btnGuardarBase').innerText='💾 ACTUALIZAR BASE';
    calc();
  } else {
    // producto compuesto
    setCrear('producto');
    ...
    document.getElementById('prodCategoria').value=p.categoria||'general';
    document.getElementById('margen2').value=p.margen||100;
    document.getElementById('ventaManual2').value='';
    document.getElementById('basesSel').innerHTML='';
    if(p.receta && p.receta.length){
      p.receta.forEach(r=> addBase({id:r.baseId}));
    } else { addBase(); }
    ...
    document.getElementById('tituloProd').innerText='Producto - Editando: '+p.nombre;
    document.getElementById('btnGuardarProd').innerText='💾 ACTUALIZAR - Mover de categoría';
    calc2();
  }
}
...
function renderInventario(){
  let ps=getProd();
  document.getElementById('listaInv').innerHTML=ps.map(p=>{
    let catNombre=getCategoriasVenta().find(c=>c.id==p.categoria)?.nombre||p.categoria||'General';
    if(p.categoria=='base') catNombre='BASE';
    let tipoBadge=p.esBase!==false && (p.categoria=='base' || p.esBase)? '<span class="bg-orange-100 text-orange-700 text-[8px] px-2 py-0.5 rounded-full font-black">RECETA BASE</span>' : `<span class="bg-blue-100 text-blue-700 text-[8px] px-2 py-0.5 rounded-full font-black">${catNombre}</span>`;
    return `<div class="bg-white p-3 rounded-2xl flex gap-2 shadow-sm mt-3 border items-center"><div class="flex-1"><div class="flex gap-1 items-center">${tipoBadge}</div><b class="text-[13px]">${p.nombre}</b><br><span class="text-[11px]">$${p.costo.toFixed(2)} → $${p.venta.toFixed(2)} ${p.receta? '• '+(p.receta.length)+' ingredientes/bases':''}</span></div><div class="flex flex-col gap-1"><button onclick="editarProd(${p.id})" class="bg-blue-500 text-white w-8 h-8 rounded-full text-[12px] font-black">✏️</button><button onclick="let pr=getProd().filter(x=>x.id!=${p.id}); localStorage.setItem('productosV2',JSON.stringify(pr)); renderInventario(); renderVenta();" class="bg-red-100 text-red-500 w-8 h-8 rounded-full font-black">X</button></div></div>`;
  }).join('')||'<p class="text-center text-gray-400 py-4 text-[11px]">Sin recetas. Crea tu primera BASE</p>';
}
function renderVenta(){ let ps=getProd().filter(p=>p.esBase===false || p.categoria!='base'); let cats=getCategoriasVenta(); if(categoriaFiltro!='todas') ps=ps.filter(p=>p.categoria==categoriaFiltro); if(!ps.length){ document.getElementById('listaVenta').innerHTML='<p class="col-span-2 text-center text-gray-400 text-[12px] py-10">Sin productos en esta categoría<br><span class="text-[10px]">Crea uno y asígnalo a esta categoría o cambia el filtro</span></p>'; return; } document.getElementById('listaVenta').innerHTML=ps.map(p=>`<div class="bg-white rounded-2xl shadow-sm border p-3"><div class="flex justify-between items-start"><div class="text-[8px] bg-black text-white px-2 py-0.5 rounded-full inline-block mb-1">${cats.find(c=>c.id==p.categoria)?.nombre||p.categoria||'General'}</div><button onclick="editarProd(${p.id})" class="text-[10px] bg-gray-100 px-2 rounded-full">✏️</button></div><b class="text-[13px] block mt-1">${p.nombre}</b><p class="text-green-600 font-black">$${p.venta.toFixed(2)}</p><button onclick="addCart(${p.id})" class="w-full mt-2 bg-black text-white py-2 rounded-xl text-[11px]">Agregar</button></div>`).join(''); }
...
function renderClientes(){ let cli=getCli(); document.getElementById('listaClientes').innerHTML=cli.map(c=>`<div class="bg-gray-50 p-3 rounded-xl border flex justify-between"><div><b>${c.nombre}</b> ${c.tel}</div><button onclick="let cl=getCli().filter(x=>x.id!='${c.id}'); localStorage.setItem('clientesV2',JSON.stringify(cl)); renderClientes();" class="text-red-400">X</button></div>`).join(''); }
...
showTab('costos'); renderFijos(); cargarEmpresa(); renderCategoriasVenta(); renderProdCategoriaSelect(); renderInventario();
</script>
</body></html>
    """
