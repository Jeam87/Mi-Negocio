...
<title>Mi Negocio</title><script src="https://cdn.tailwindcss.com"></script>
...
function calc2(){
 ...
 document.querySelectorAll('#basesSel > div').forEach(row=>{
  ...
  let b=prods.find(x=>String(x.id)==String(sel.value)); if(!b) return;
 ...
 let cEl=document.getElementById('costoTxt'); if(cEl) cEl.innerText='$'+tot.toFixed(2);
 let margen=parseFloat(document.getElementById('margenInput')?.value)||100;
 let venta=tot*(1+margen/100);
 let f=document.getElementById('finalInput')?.value; if(f&&f.trim()!==''){let mv=parseCant(f); if(mv>0) venta=mv;}
 let vEl=document.getElementById('ventaTxt'); if(vEl) vEl.innerText='$'+venta.toFixed(2);
}
function addBaseRow(d){
 d=d||{}; let bases=prods.filter(x=>x.tipo==='base'||x.esBase);
...
function guardarBase(){
 ...
 if(editBaseId){ let b=prods.find(x=>x.id==editBaseId); if(b){b.nombre=nombre; b.costo=costo; b.rendimiento={cant:rCant,unit:rUnit}; b.stock=stock; b.categoria=cat;} editBaseId=null; }
...
function editarBase(id){ let b=prods.find(x=>x.id==id); if(!b) return; editBaseId=id; setTab('invent'); setTimeout(()=>{document.getElementById('ib-nombre').value=b.nombre; document.getElementById('ib-costo').value=b.costo; document.getElementById('ib-rinde-cant').value=b.rendimiento.cant; document.getElementById('ib-rinde-unit').value=b.rendimiento.unit; document.getElementById('ib-stock').value=b.stock||0; document.getElementById('ib-cat').value=b.categoria||'Salsas';},120); }
...
function render(){
 ...
 if(tab==='crear'){
  ...
  <div class="card p-3 bg-[#FFFBE6] mt-3"><div class="font-black mb-1">Bases (usa 0.2L, 150g, 1/4) → solo cobra esa parte</div><div id="basesSel"></div><button onclick="addBaseRow()" class="w-full border-2 border-black rounded-[16px] p-3 bg-white font-bold mt-2">+ Salsa / Papa</button></div>
 ...
 if(tab==='catalogo'){ html+=`<div class="font-black mb-2">Catalogo (${secund.length})</div>${secund.map(p=>`<div class="bg-white border-2 border-black rounded-xl p-3 mb-2"><b>${p.nombre}</b> $${(p.costo||0).toFixed(2)} → $${(p.venta||0).toFixed(2)}<br><span class="text-[11px] opacity-60">${p.bases?.map(bb=>{let b=prods.find(x=>x.id==bb.id); return (b?b.nombre:'')+' '+bb.cant+bb.unit}).join(', ')}</span></div>`).join('')}`; }
 if(tab==='finanzas'){ html+=`<div class="card p-4">Finanzas: ${secund.length} productos</div>`; }
 if(tab==='clientes'){ html+=`<div class="card p-4">Clientes</div>`; }
 document.getElementById('app').innerHTML=html; if(tab==='crear') calc2();
 document.querySelectorAll('nav button').forEach(b=>b.classList.remove('tab-active')); let el=document.getElementById('t-'+tab); if(el) el.classList.add('tab-active');
}
render();
</script></body></html>
"""
