...
def manifest(): return jsonify({"name":"Mi Negocio 10.3","short_name":"Mi Negocio","start_url":"/","display":"standalone"})
@app.route('/logo.png')
def logo_file():
...
def api_register():
    try:
        ...
        users=load_users()
        if email in users: return jsonify({"ok":False,"msg":"Ya existe, dale Entrar"}),400
...
<title>Mi Negocio 10.3</title><script src="https://cdn.tailwindcss.com"></script><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
...
<h1 class="font-black text-[24px]">Mi Negocio 10.3</h1><p class="text-[11px] text-gray-500">Hora celular + WhatsApp</p>
...
<h2 class="font-black">Cobrar $<span id="cobroTotal">0</span> a <span id="cobroClienteNombre" class="text-blue-600"></span></h2>
<input id="pagoRecibido" type="number" placeholder="$ Recibido" class="w-full border-2 border-black p-3 rounded-xl mt-3 font-black text-[18px]" oninput="calcCambio()">
<p class="mt-2 font-black">Cambio: $<span id="cambio">0.00</span> • <span id="vendedorCobro" class="text-blue-600"></span> • <span id="cobroHora" class="text-green-600"></span></p>
<div class="grid grid-cols-2 gap-2 mt-4">
<button onclick="confirmarCobro('print')" class="bg-black text-white py-4 rounded-2xl font-black">🖨️ COBRAR + IMPRIMIR</button>
<button onclick="confirmarCobro('whatsapp')" class="bg-[#25D366] text-white py-4 rounded-2xl font-black">📲 COBRAR + WHATSAPP</button>
</div>
<button onclick="confirmarCobro('solo')" class="w-full mt-2 bg-gray-100 py-3 rounded-xl font-bold">Solo cobrar</button>
...
function getFechaLocal(){
  // Usa la hora REAL del celular, no del servidor
  let now=new Date();
  return now.toLocaleString('es-MX', {day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:true});
}
...
function actualizarHora(){ document.getElementById('horaActual').innerText='🕒 '+getFechaLocal(); }
...
function actualizarClienteTicket(){
  let sel=document.getElementById('selCliente'); if(!sel) return;
  let opt=sel.options[sel.selectedIndex];
  let nombre=opt? (opt.getAttribute('data-nombre')||opt.text) : 'Mostrador';
  let el=document.getElementById('cobroClienteNombre'); if(el) el.innerText=nombre;
}
...
function generarTicket(data){
  ...
  let html=`${logoHtml}<div style="text-align:center; border-bottom:1px dashed #000; padding-bottom:8px;"><b>${emp.nombre}</b><br><span style="font-size:9px">${emp.direccion||''} ${emp.tel||''}</span></div><div style="font-size:10px;">Fecha: ${fechaHora}<br>Cliente: ${data.cliente}<br>Vendedor: ${data.vendedor||currentUser}<br>Hora cel: ${getFechaLocal()}</div><div style="border-top:1px dashed #000; border-bottom:1px dashed #000; padding:6px 0; margin:6px 0;">${data.items.map(i=>`<div style="display:flex; justify-content:space-between;"><span>${i.nombre} x${i.qty}</span><span>$${(i.venta*i.qty).toFixed(2)}</span></div>`).join('')}</div><div style="display:flex; justify-content:space-between; font-weight:bold;"><span>TOTAL</span><span>$${data.total.toFixed(2)}</span></div><div style="text-align:center; margin-top:12px; border-top:1px dashed #000; padding-top:8px;">${emp.mensaje}</div>`;
...
function enviarWhatsAppDirecto(tel,msg){ if(!tel){ tel=prompt('Numero WhatsApp'); if(!tel) return; } tel=tel.toString().replace(/\\D/g,''); if(tel.length==10) tel='52'+tel; if(tel.length==12 && tel.startsWith('52')){} let url=`https://wa.me/${tel}?text=${encodeURIComponent(msg||'Hola')}`; window.open(url,'_blank'); }
function enviarWhatsAppTicket(esPrueba){
  if(!ultimoTicket && esPrueba){ actualizarVistaTicket(); ultimoTicket={fechaStr:getFechaLocal(),items:[{nombre:'Alitas',qty:2,venta:57}],total:114,cliente:'Mostrador',vendedor:currentUser}; }
  if(!ultimoTicket) return alert('No hay ticket');
  let sel=document.getElementById('selCliente');
  let opt=sel? sel.options[sel.selectedIndex] : null;
  let tel=opt? opt.getAttribute('data-tel') : '';
  if(esPrueba &&!tel){ tel=prompt('Numero WhatsApp para prueba (10 dígitos)'); }
  if(!tel){
    let cliId=sel? sel.value : '';
    let cli=getCli().find(c=>c.id==cliId);
    tel=cli? cli.tel : '';
  }
  if(!tel){ tel=prompt('Escribe el WhatsApp del cliente (10 dígitos):'); if(!tel) return; }
  tel=tel.toString().replace(/\\D/g,'');
  if(tel.length==10) tel='52'+tel;
  let texto=generarTextoWhatsApp(ultimoTicket);
  let url=`https://wa.me/${tel}?text=${encodeURIComponent(texto)}`;
...
function renderCarrito(){ if(!carrito.length){ document.getElementById('ticket').innerHTML='Vacío - agrega productos'; document.getElementById('c-total').innerText='0'; return; } let t=0,h=''; carrito.forEach((x,idx)=>{ t+=x.venta*x.qty; h+=`<div class="flex justify-between bg-gray-50 p-2 rounded-xl"><span>${x.nombre} x${x.qty}</span><span>$${(x.venta*x.qty).toFixed(0)} <button onclick="carrito.splice(${idx},1); renderCarrito();" class="text-red-500">X</button></span></div>`; }); document.getElementById('ticket').innerHTML=h; document.getElementById('c-total').innerText=t.toFixed(0); document.getElementById('cobroTotal').innerText=t.toFixed(0); }
function abrirCobro(){ if(!carrito.length) return alert('Vacío'); let sel=document.getElementById('selCliente'); actualizarClienteTicket(); document.getElementById('cobroHora').innerText=getFechaLocal(); document.getElementById('modalCobro').classList.remove('hidden'); }
...
function confirmarCobro(tipo){
  ...
  facts.push({id:Date.now(),concepto:'Venta: '+carrito.map(c=>c.nombre+' x'+c.qty).join(', ')+' | Cliente: '+clienteNombre,monto:total,fecha:fechaSolo,fechaHora:fechaStr,tipo:'entrada',vendedor:currentUser||'dueño',clienteId,clienteNombre,clienteTel});
  setItem('facturas',facts);
  ultimoTicket={fecha:new Date(),fechaStr:fechaStr,items:[...carrito],total,cliente:clienteNombre,clienteTel:clienteTel,vendedor:currentUser};
  generarTicket(ultimoTicket);
  cerrarCobro();
  if(tipo=='print'){ imprimirTicket(); }
  else if(tipo=='whatsapp'){
    let tel=clienteTel;
    if(!tel || tel==''){ tel=prompt('WhatsApp del cliente (10 dígitos) para enviar ticket:'); }
    if(tel){
      tel=tel.toString().replace(/\\D/g,''); if(tel.length==10) tel='52'+tel;
      let texto=generarTextoWhatsApp(ultimoTicket);
